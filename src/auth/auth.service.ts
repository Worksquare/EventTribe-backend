...
import {
  ...
  export class AuthService {
    ...
    async login(loginDto: CreateLoginDto): Promise<any> {
      ...
      try {
        ...
        const userInfo = {
          ...
          isVerified: userExist.isVerified,
          emailVerificationTokenExpiresAt: userExist.emailVerificationTokenExpiresAt
        }
        return {
          responseCode: 200,
          userInfo,
    ...
    async emailVerification(emailVerificationDto: EmailVerificationDto): Promise<any> {
      const {token} = emailVerificationDto
      try {
        console.log('The requesting token is:', token)
        const secret = process.env.ACCESS_TOKEN_SECRET;
        ...
        const tokenExpirationTime = userExist.emailVerificationTokenExpiresAt?.getTime() || 0;
    ...
    async resetPasswordLink(resetPasswordLinkDto: ResetPasswordLinkDto): Promise<any> {
