...
import { Controller, Post, Body, UseGuards, Get, Req, HttpCode, Res, Param, HttpStatus} from '@nestjs/common';
import { AuthService } from './auth.service';
import { AuthGuard } from "@nestjs/passport";
import { CreateUserDto } from '../dto/signup.dto';
import { CreateLoginDto } from '../dto/login.dto';
import { Request, Response } from "express";
import { ResetPasswordLinkDto, ResetPasswordDto } from '../dto/resetPassword.dto';
...
export class AuthController {
  ...
  @ApiBadRequestResponse({ description: 'User with the email and password already exists.' })
  @ApiInternalServerErrorResponse({ description: 'Internal server error' })
  ...
  @Post('/reset/:token')
  @HttpCode(HttpStatus.OK)
  @ApiOkResponse({ type:ResetPasswordDto, description: 'Reset password is successfully' })
  ...
  @Get('/resetLink')
  @HttpCode(HttpStatus.OK)
  @ApiOkResponse({ type:ResetPasswordDto, description: 'Reset password successfully sent' })
  @ApiBadRequestResponse({ description: 'Invalid email.'})
  @ApiInternalServerErrorResponse({ description: 'Internal server error' })
    async resetPasswordLink(@Body() resetPasswordLinkDto: ResetPasswordLinkDto): Promise<string> {
  ...
  @Get('/verify/:token')
  @HttpCode(HttpStatus.OK)
  @ApiOkResponse({ type:ResetPasswordDto, description: 'The account has been verified successfully.' })
  @ApiBadRequestResponse({ description: 'Invalid token.'})
  @ApiInternalServerErrorResponse({ description: 'Internal server error' })
  async emailVerification(@Body() emailVerificationDto: EmailVerificationDto): Promise<string> {
